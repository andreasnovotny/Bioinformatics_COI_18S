...
#the raw data may be in paired end (use standard pipline below) or non-paired (skip steps for reverse reads, code will need modification to exclude references to reverse reads) format, but each sample should have a forward and reverse (if applicable) read file associated
...
#at this point, you are ready to do BLAST-based taxonomy assignment, if you wish. please see "assign_taxonomy.all_amplicons.BLAST.sh" for the code for that process